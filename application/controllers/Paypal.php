...
class Paypal extends CI_Controller{ 
    ...
    function success(){ 
        ...
        if(!empty($paypalInfo['item_number']) && !empty($paypalInfo['tx']) && !empty($paypalInfo['amt']) && !empty($paypalInfo['cc']) && !empty($paypalInfo['st'])){ 
            ...
            semdEmail($this->session->userdata['email'],'Successfully payment done','Payment plan has been successfully acitivated');
            $productData = $this->plans_model->getRows($item_number); 
             
            // Check if transaction data exists with the same TXN ID 
            $paymentData = $this->plans_model->getPayment(array('txn_id' => $txn_id)); 
        } 
     ...
     function cancel(){ 
        ...
        semdEmail($this->session->userdata['email'],'Payment failed','Payment has been cancelled due to technical reasons');
