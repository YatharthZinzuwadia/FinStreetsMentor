...
class Message_model extends CI_Model {
    ...
    public function getByuser($type, $sender_id, $reciverId) {
        if ($type == SENDER) {
            $this->db->select('user.*,message.*');
            $this->db->join('user', 'user.user_id = message.reciver_id');
            $this->db->where("(message.sender_id=$sender_id AND message.reciver_id=$reciverId) OR (message.sender_id=$reciverId AND message.reciver_id=$sender_id)");
//            $sql =  $this->db->get_where('message',['message.sender_id'=>$sender_id,'message.reciver_id'=>$user_id]);
            $this->db->order_by('send_on', 'ASC');
...
//            echo $this->db->last_query();
    ...
    public function getMessage($user_id) {
        $this->db->select('user.*,message.*');
        $this->db->join('user', 'user.user_id = message.sender_id');
        return $this->db->get_where('message', ['message.reciver_id' => $user_id])->result_array();
    }
